...
import '../posts/public/public_image_posts_list.dart';
import '../posts/public/public_things_questions_list.dart';
...
class _PublicProfileState extends State<PublicProfile>
  ...
  bool _isPreviewingCover = false;

  final Map<String, Uint8List> _avatarBytesCache = {};

  Uint8List? _getCachedAvatarBytes(String? base64) {
    if (base64 == null || base64.isEmpty) return null;
    if (_avatarBytesCache.containsKey(base64)) return _avatarBytesCache[base64];
    final bytes = getConvertImage(base64);
    if (bytes != null) _avatarBytesCache[base64] = bytes;
    return bytes;
  }
      ...
      WidgetsBinding.instance.addPostFrameCallback((_) {
        if (mounted && !_imagesInitialized) {
          ...
          if (profile.profileThumbnailUrl != null &&
            ...
            profileBytes = getConvertImage(profileUrl);
          ...
          if (profile.coverThumbnailUrl != null &&
            ...
            coverBytes = getConvertImage(coverUrl);
          }
  ...
  void _showImagePreview(Uint8List? bytes, {bool isCover = false}) {
    if (bytes == null) return;
    setState(() {
      _previewImageBytes = bytes;
      _isPreviewingCover = isCover;
    });
  }
              ...
              SliverToBoxAdapter(
                child: Consumer<PublicProfileProvider>(
                  builder: (context, publicProfileProvider, child) {
                    ...
                    final canViewPosts = _canViewPosts(
                      isFriend,
                      profile.isPrivate,
                    );
                      ...
                      children: [
                        RepaintBoundary(
                          child: GestureDetector(
                            onTap: () {
                              // Get cover image bytes from profile
                              final coverBytes =
                                  _cachedCoverImageBytes ??
                                  getConvertImage(profile.coverThumbnailUrl);
                              if (coverBytes != null) {
                                _showImagePreview(coverBytes, isCover: true);
                              } else {
                                showToast(message: 'No cover photo');
                              }
                            },
                            child: Container(
                              height: 180.h,
              ...
              child: Stack(
                ...
                children: [
                  ...
                  GestureDetector(
                    onTap: _closeImagePreview,
                    child: Hero(
                      tag: _isPreviewingCover ? 'cover_image' : 'profile_image',
                      child: PhotoView(
                        imageProvider: MemoryImage(_previewImageBytes!),
                        minScale: PhotoViewComputedScale.contained * 0.8,
                        maxScale: PhotoViewComputedScale.covered * 4.0,
                        // Use 'contained' so it shows the full image at natural aspect ratio
                        initialScale: PhotoViewComputedScale.contained,
                        basePosition: Alignment.center,
                        // This is key — don't force the widget to fill the screen
                        tightMode: false,
