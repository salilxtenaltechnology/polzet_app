...
class _AllImagesPopupState extends State<AllImagesPopup> {
  ...
  void toggleImageSelection(int index) {
    ...
    setState(() {
      if (selectedImages.containsKey(index)) {
        final removedOrder = selectedImages[index]!;
        selectedImages.remove(index);
        final reorderedMap = <int, int>{};
  ...
  Future<void> submitPollVotes() async {
    ...
    try {
      ...
      selectedImages.forEach((imageIndex, rank) {
        votes.add({'option_id': widget.images[imageIndex].id, 'rank': rank});
      });
      int nextRank = selectionCounter + 1;
      for (int i = 0; i < widget.images.length; i++) {
        if (!selectedImages.containsKey(i)) {
          votes.add({'option_id': widget.images[i].id, 'rank': nextRank});
          nextRank++;
        }
      }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: SafeArea(
        child: Column(
          children: [_buildHeader(), _buildImageList(), _buildPollButton()],
        ),
      ),
    );
  }
    ...
    return Expanded(
      child: ListView.builder(
        ...
        itemBuilder: (context, index) {
          final option = widget.images[index];
          ...
          final topVoters = _topVotersMap[option.id] ?? [];
